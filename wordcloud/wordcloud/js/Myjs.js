...
function VisualizeWordCloud(trips){
	...
	for (var i=0; i<trips.length; i++) {
		for (var j=0; j<15; j++) {
			array.push(trips[i].streetnames[j]);
		}
	}
	
	var words = sortByFrequency(array).map(function(d,j) {
...
var fRatio = Math.min(cWidth, cHeight) / ctx.measureText(words[0].text).width,
	fontScale = d3.scale.linear()
		.domain([
			d3.min(words, function(j) { return j.size; }), 
			d3.max(words, function(j) { return j.size; })
		])
		//.range([20,120]),
		.range([10,35]), // tbc
